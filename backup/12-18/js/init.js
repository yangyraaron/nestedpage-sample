			...
			if (data.length) {
				...
				function itemclick (item,grid,data) {
					item.click(function () {
						...
						var activeItems = grid.find('.ui-li.ui-btn-active');

						if(activeItems.length){
							for(var i in activeItems){
								var ai = activeItems[i];

								if(ai.id !== item.attr('id'))
									$('#'+ai.id).removeClass('ui-btn-active');
							}
						}
				...
				for (var index in data) {
					...
					var item = $("<li id='"+key+"'><a href='" + this.options.linkUrl + "'>"+
								"<img class='cs-img' style='margin:0.6em 0;' width='60px' height='60px' cs-baseUrl='"+baseUrl+"' cs-url='"+imgUrl+"' />"+
...
function loadBizs($, wind, doc) {
	...
	(function detail_page() {
		...
		var showStatic = function () {
			...
			var current = grid.selectedItem;//$("#news_ul").jqmData("current_item");
		...
		function showImg (img,url,width,height) {
			img.click(function () {
				...
				pop.append("<img src='"+url+"' width='"+width+"' height='"+height+"' />");
