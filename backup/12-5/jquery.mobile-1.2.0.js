		...
		(function ($, window, undefined) {
			...
			var createHandler = function (sequential) {
				...
				return function (name, reverse, $to, $from) {
					...
					slideTransition = {
						createTransfrom:function (x) {
							return 'translateX(' + x + '%)';
						},
						_transition:'-webkit-transform 0.35s ease-out',
						slideOut:function (el) {
							el.css({'webkitTransitionProperty':this._transition,
								'webkitTransform':this.createTransfrom('-100')});
						},
						slideIn:function (el) {
							el.css({'webkitTransitionProperty':this._transition,
									'webkitTransform':this.createTransfrom('0')});
						}
					},
					scrollPage = function () {
					...
					startOut = function () {
						...
						if (!sequential) {
							...
							transitionComplete($from,doneOut);
						}
					...
					startIn = function () {
						...
						//toPreClass:"ui-page-prev-in"
						$to.addClass($.mobile.activePageClass + toPreClass)
						.css('webkitTransform','translateX(100%)');
					...
					transitionComplete = function (el,complete) {
						...
						// 	alert('webkitTransitionEnd');
						// 	if(complete) complete();
						// 	target.removeEventListener('webkitTransitionEnd',evt,false);
						// };

						// target.addEventListener('webkitTransitionEnd',evt,false);
