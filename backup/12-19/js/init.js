(function ($, wind, doc) {
	...
	database.prototype = {
		_errHandler : function (func, sql, err) {
			...
			$.fm.logger.logError("error:executing sql " + sql + " of " + func + " on database " + this.options.name + " error:" + msg);
		},
		_exec : function (func, sql, onComplete) {
			var $this = this;
			
			$.fm.logger.logInfo('begin to excute sql: '+sql);
				...
				tx.executeSql(sql, [],
					...
					if (raf) {
						...
						for (var i = 0; i < len; i++) {
							...
							$.fm.logger.logInfo('results item: ',d);
					...
					$.fm.logger.logInfo('the result data: ',data);
					onComplete(data);
	...
	$.fm.device = {
		...
		_execReady:function () {
			...
			$.fm.logger.logInfo('device ready');
	...
	function initDb () {
		$.fm.logger.logInfo('intialize the database ');
	...
	$.fm.device.ready(initDb);

	$.fm.device._deviceReady();
	// $.fm.db = {
	// 	getdb : function (db, options) {
	// 		return new database(db, options);
	// 	}
		...
		addNavs : function (navs) {
			...
			var navClick = function (el, nav) {
				el.click(function () {
					...
					$.fm.logger.logInfo("navigation click: ",nav.title,state);
					...
					if (nav.url !== $this.firstPage.url) {
						if (nav.url && state.filename !== nav.url) {
							...
							$.fm.logger.logInfo('change page to:'+nav.url);
						...
						$.fm.logger.logInfo('change page to first page: '+firstPage.attr('id') );
...
(function ($, wind, doc) {
	...
	$.fm.logger ={
			...
			_log:function (args) {
				if($.fm.utility.isString(args)){
					this.logatomic(args);
				}else if($.fm.utility.isArray(args)){
					var that = this;
				...
				}else if($.fm.utility.isObject(args)){
					this.logObj(args);
				}
...
function createWidgets($, wind, doc) {
	...
	$.widget("news.datagrid", {
		...
		_render : function (data, options) {
			$.fm.logger.logInfo('begin to render data');
			...
			$.fm.logger.logInfo('finish render data');
		},
		_firstLoad : true,
		_getDataServer : function (options) {
			var $this = this;

			$.fm.logger.logInfo('get data from server','pageNo: '+options.data.pageNo);
				...
				success : function (data) {
					if (data && data.jsonp && data.jsonp.data && data.jsonp.data.status === 1) {
						...
						if ($.fm.db) {
							...
							$.fm.logger.logInfo('get news db:',db);

							$.fm.logger.logInfo('begin add news:',news,' to db: ')
							...
							db.add("NEWS", {
								...
								onComplete : function (ar) {
									if (ar.length) {
										$.fm.logger.logInfo("add the " + ar[0] + "data to news db");
									}
									$.fm.logger.logInfo('finish add data to news');
								}
							});
						}
		...
		_getData : function (options) {
			...
			if ($.fm.db.isTrueReady && options.caching && $.fm.db && this._firstLoad) {
				var db = $.fm.db.getdb("news");
				...
				$.fm.logger.logInfo('create news db');
				$.fm.logger.logInfo('begin get news from database');
				...
				db.query("NEWS", {
					...
					onComplete : function (data) {
						$.fm.logger.logInfo('finish get data: '+ data +' from db: ');
						...
						if (data.length) {
							...
							$.fm.logger.logInfo('no data was found in db');
...
function loadBizs($, wind, doc) {
	...
	(function news_page() {
		...
		$("#news_page").live("pageshow", function (event) {
			...
			$.fm.device.ready(initNews);

			$.fm.device._simulateReady();
