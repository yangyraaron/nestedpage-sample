...
function createWidgets($, wind, doc) {
	...
	$.widget("news.datagrid", {
		...
		_render : function (data, options) {
			$.mex.logger.logInfo('begin to render data');
			...
			$.mex.logger.logInfo('finish render data');
		},
		_firstLoad : true,
				...
				success : function (data) {
					if (data && data.jsonp && data.jsonp.data && data.jsonp.data.status === 1) {
						...
						if ($.mex.db) {
							...
							$.mex.logger.logInfo('begin add news:',news,' to db: ')
							...
							db.add("NEWS", {
								...
								onComplete : function (ar) {
									...
									$.mex.logger.logInfo('finish add data to news');
								}
							});
						}
		...
		_getData : function (options) {
			...
			if ($.mex.db.isTrueReady && options.caching && $.mex.db && this._firstLoad) {
				var db = $.mex.db.getdb("news");
