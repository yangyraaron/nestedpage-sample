(function ($, wind, doc) {
	...
	database.prototype = {
		_errHandler : function (func, sql, err,onerror) {
			...
			$.mex.logger.logError("error:executing sql " + sql + " of " + func + " on database " + this.options.name + " error:" + msg);
		...
		_exec : function (func, sql, onComplete,onerror) {
			...
			$.mex.logger.logInfo('begin to excute sql: '+sql);
				...
				tx.executeSql(sql, [],
					...
					if (raf) {
						...
						for (var i = 0; i < len; i++) {
							...
							m = $.mex.utility.getObjInfo(d),
							mlen = m.length;
							
							$.mex.logger.logInfo('results item: ',d);
					...
					$.mex.logger.logInfo('the result data: ',data);
					onComplete(data);

				}, function (err) {
					$this._errHandler("executeSql error: " + func, sql, err,onerror);
				});
			},
				function (err) {
				$this._errHandler("transaction error: " + func, sql, err,onerror);
			});
		...
		_filter : function (filter) {
			var sql = ' WHERE ';
			if (filter) {
				var mtx = $.mex.utility.getObjInfo(filter);
	...
	$.mex.device = {
		...
		_execReady:function () {
			...
			$.mex.logger.logInfo('device ready');
	...
	function initDb () {
		$.mex.logger.logInfo('intialize the database ');
	...
	$.mex.device.ready(initDb);

	$.mex.device._deviceReady();
	// $.mex.db = {
	// 	getdb : function (db, options) {
	// 		return new database(db, options);
	// 	}
	...
	var deferred = new $.Deferred(),
		...
		prepare = function () {
			...
			if(!none && $from){
				...
				if(!reverse){
					var value = $to.css('-webkit-transform'),
			...
			cleanFrom = function() {
				$from.css('-webkit-transform','none');
		...
		addNavs : function (navs) {
			...
			var navClick = function (el, nav) {
				el.click(function () {
					...
					$.mex.logger.logInfo("navigation click: ",nav.title,state);
					...
					if (nav.url !== $this.firstPage.url) {
						if (nav.url && state.filename !== nav.url) {
							...
							$.mex.logger.logInfo('change page to:'+nav.url);
						...
						$.mex.logger.logInfo('change page to first page: '+firstPage.attr('id') );
					...
					$.mex.shell.curNav = nav;
					$.mex.shell.setTitle(nav.title);
				});
			};
...
(function ($, wind, doc) {
	...
	$.mex.logger ={
			...
			_log:function (args) {
				if($.mex.utility.isString(args)){
					this.logatomic(args);
				}else if($.mex.utility.isArray(args)){
					var that = this;
				...
				}else if($.mex.utility.isObject(args)){
					this.logObj(args);
				}
...
function createWidgets($, wind, doc) {
	...
	$.widget("news.datagrid", {
		...
		_render : function (data, options) {
			$.mex.logger.logInfo('begin rendering data');
			...
			if (data.length) {
				...
				for (var index=0;index<data.length;index++) {
					...
					$.mex.utility.loadImg(img[0],dPath);
			...
			$.mex.logger.logInfo('finish rendering data');
		},
		_firstLoad : true,
		_getDataServer : function (options) {
			var $this = this;

			$.mex.logger.logInfo('get data from server','pageNo: '+options.data.pageNo);
			...
			$.ajax({
				...
				success : function (data) {
					if (data && data.jsonp && data.jsonp.data && data.jsonp.data.status === 1) {
						...
						if ($.mex.db) {
							var db = $.mex.db.getdb("news");

							$.mex.logger.logInfo('get news db:',db);
							$.mex.logger.logInfo('begin adding news:',news,' to db: ')
							...
							db.add("NEWS", {
								...
								onComplete : function (ar) {
									if (ar.length) {
										$.mex.logger.logInfo("add the " + ar[0] + "data to news db");
									}
									$.mex.logger.logInfo('finish adding data to news');
								},
								onerror:function(err){
									$.mex.logger.logInfo('add news failed!');
								}
							});
						}
		...
		_getData : function (options) {
			...
			if ($.mex.device.isReady && options.caching && this._firstLoad) {
				var db = $.mex.db.getdb("news");
				...
				$.mex.logger.logInfo('create news db');
				$.mex.logger.logInfo('begin get news from database');
				...
				db.query("NEWS", {
					...
					onComplete : function (data) {
						$.mex.logger.logInfo('finish get data: '+ data +' from db: ');
						...
						if (data.length) {
							...
							$.mex.logger.logInfo('no data was found in db');

							$this._getDataServer(options);
						}
					},
					onerror:function(err){
						$.mex.logger.logInfo('query news failed!');
					...
					success:function (data) {
								...
								if(imgs&&imgs.length){
									...
									for(var i=0;i<imgs.length;i++){
										...
										$.mex.utility.loadImg($img[0],'css/images/default.png');
